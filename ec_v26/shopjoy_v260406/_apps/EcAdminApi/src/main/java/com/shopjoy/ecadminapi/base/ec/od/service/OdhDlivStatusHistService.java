...
public class OdhDlivStatusHistService {
    ...
    @Transactional(readOnly = true)
    public PageResult<OdhDlivStatusHistDto> getPageData(Map<String, Object> p, int pageNo, int pageSize) {
        p = new HashMap<>(p);
        int offset = (pageNo - 1) * pageSize;
        p.put("limit", pageSize);
        p.put("offset", offset);
        long totalCount = mapper.selectPageCount(p);
        List<OdhDlivStatusHistDto> pageList = mapper.selectPageList(p);
        PageResult<OdhDlivStatusHistDto> result = PageResult.of(pageList, totalCount, pageNo, pageSize, p);
        return result;
    }
