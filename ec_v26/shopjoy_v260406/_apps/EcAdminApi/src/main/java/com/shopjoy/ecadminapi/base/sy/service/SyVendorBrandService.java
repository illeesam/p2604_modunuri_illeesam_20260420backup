...
public class SyVendorBrandService {
    ...
    @Transactional(readOnly = true)
    public PageResult<SyVendorBrandDto> getPageData(Map<String, Object> p, int pageNo, int pageSize) {
        p = new HashMap<>(p);
        int offset = (pageNo - 1) * pageSize;
        p.put("limit", pageSize);
        p.put("offset", offset);
        long totalCount = mapper.selectPageCount(p);
        List<SyVendorBrandDto> pageList = mapper.selectPageList(p);
        PageResult<SyVendorBrandDto> result = PageResult.of(pageList, totalCount, pageNo, pageSize, p);
        return result;
    }
    ...
    public void delete(String id) {
        if (!repository.existsById(id))
            throw new CmBizException("존재하지 않는 SyVendorBrand입니다: " + id);
