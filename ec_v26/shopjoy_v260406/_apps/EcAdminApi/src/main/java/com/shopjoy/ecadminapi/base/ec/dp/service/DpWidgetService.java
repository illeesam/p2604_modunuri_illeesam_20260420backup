...
public class DpWidgetService {
    ...
    @Transactional(readOnly = true)
    public PageResult<DpWidgetDto> getPageData(Map<String, Object> p, int pageNo, int pageSize) {
        p = new HashMap<>(p);
        int offset = (pageNo - 1) * pageSize;
        p.put("limit", pageSize);
        p.put("offset", offset);
        long totalCount = mapper.selectPageCount(p);
        List<DpWidgetDto> pageList = mapper.selectPageList(p);
        PageResult<DpWidgetDto> result = PageResult.of(pageList, totalCount, pageNo, pageSize, p);
        return result;
    }
