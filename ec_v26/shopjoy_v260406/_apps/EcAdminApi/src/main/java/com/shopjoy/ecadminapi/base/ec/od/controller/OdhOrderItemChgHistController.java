...
public class OdhOrderItemChgHistController {
    ...
    public ResponseEntity<ApiResponse<PageResult<OdhOrderItemChgHistDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<OdhOrderItemChgHistDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<OdhOrderItemChgHistDto>> getById(@PathVariable String id) {
