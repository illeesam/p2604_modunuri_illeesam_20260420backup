...
public class OdhPayChgHistController {
    ...
    public ResponseEntity<ApiResponse<PageResult<OdhPayChgHistDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<OdhPayChgHistDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<OdhPayChgHistDto>> getById(@PathVariable String id) {
