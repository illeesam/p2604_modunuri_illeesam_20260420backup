...
public class PdProdSetItemController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<PdProdSetItemDto>>> list(
        ...
        List<PdProdSetItemDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<PdProdSetItemDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<PdProdSetItemDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PdProdSetItemDto>> getById(@PathVariable String id) {
        PdProdSetItemDto result = service.getById(id);
    ...
    public ResponseEntity<ApiResponse<PdProdSetItem>> create(@RequestBody PdProdSetItem entity) {
        PdProdSetItem result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<PdProdSetItem>> save(
            @PathVariable String id, @RequestBody PdProdSetItem entity) {
        entity.setSetItemId(id);
        PdProdSetItem result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
