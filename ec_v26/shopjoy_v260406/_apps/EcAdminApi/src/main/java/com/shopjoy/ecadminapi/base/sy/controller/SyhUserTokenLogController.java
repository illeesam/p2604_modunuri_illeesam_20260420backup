...
public class SyhUserTokenLogController {
    ...
    public ResponseEntity<ApiResponse<PageResult<SyhUserTokenLogDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<SyhUserTokenLogDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<SyhUserTokenLogDto>> getById(@PathVariable String id) {
