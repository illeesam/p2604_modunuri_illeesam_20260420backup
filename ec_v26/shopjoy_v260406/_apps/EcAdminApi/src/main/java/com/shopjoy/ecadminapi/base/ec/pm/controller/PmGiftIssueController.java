...
public class PmGiftIssueController {
    ...
    public ResponseEntity<ApiResponse<PageResult<PmGiftIssueDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<PmGiftIssueDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PmGiftIssueDto>> getById(@PathVariable String id) {
    ...
    public ResponseEntity<ApiResponse<PmGiftIssue>> save(
            @PathVariable String id, @RequestBody PmGiftIssue entity) {
