...
import com.shopjoy.ecadminapi.base.ec.cm.service.CmBltnService;
...
public class CmBltnController {

    private final CmBltnService service;
    ...
    public ResponseEntity<ApiResponse<PageResult<CmBltnDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<CmBltnDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
    ...
    public ResponseEntity<ApiResponse<CmBltn>> create(@RequestBody CmBltn entity) {
        CmBltn result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<CmBltn>> save(
            @PathVariable String id, @RequestBody CmBltn entity) {
        entity.setBlogId(id);
        CmBltn result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
