...
public class PdProdRelController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<PdProdRelDto>>> list(
        ...
        List<PdProdRelDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<PdProdRelDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<PdProdRelDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PdProdRelDto>> getById(@PathVariable String id) {
        PdProdRelDto result = service.getById(id);
    ...
    public ResponseEntity<ApiResponse<PdProdRel>> create(@RequestBody PdProdRel entity) {
        PdProdRel result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<PdProdRel>> save(
            @PathVariable String id, @RequestBody PdProdRel entity) {
        entity.setProdRelId(id);
        PdProdRel result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
