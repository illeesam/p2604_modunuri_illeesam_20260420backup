...
public class PmGiftController {
    ...
    public ResponseEntity<ApiResponse<PageResult<PmGiftDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<PmGiftDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
    ...
    public ResponseEntity<ApiResponse<PmGift>> create(@RequestBody PmGift entity) {
        PmGift result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<PmGift>> save(
            @PathVariable String id, @RequestBody PmGift entity) {
        entity.setGiftId(id);
        PmGift result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
