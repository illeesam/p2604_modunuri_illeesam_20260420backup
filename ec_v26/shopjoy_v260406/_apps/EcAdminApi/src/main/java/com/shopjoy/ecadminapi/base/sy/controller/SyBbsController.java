...
public class SyBbsController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<SyBbsDto>>> list(
        ...
        List<SyBbsDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<SyBbsDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<SyBbsDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<SyBbsDto>> getById(@PathVariable String id) {
    ...
    public ResponseEntity<ApiResponse<SyBbs>> create(@RequestBody SyBbs entity) {
        SyBbs result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<SyBbs>> save(
            @PathVariable String id, @RequestBody SyBbs entity) {
        entity.setBbsId(id);
        SyBbs result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
