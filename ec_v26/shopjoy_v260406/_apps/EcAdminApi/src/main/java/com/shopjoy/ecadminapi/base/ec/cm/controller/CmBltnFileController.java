...
import com.shopjoy.ecadminapi.base.ec.cm.service.CmBltnFileService;
...
public class CmBltnFileController {

    private final CmBltnFileService service;

    @GetMapping
    public ResponseEntity<ApiResponse<List<CmBltnFileDto>>> list(
        ...
        List<CmBltnFileDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<CmBltnFileDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<CmBltnFileDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<CmBltnFileDto>> getById(@PathVariable String id) {
        CmBltnFileDto result = service.getById(id);
    ...
    /* ── 등록 (JPA) ── */
    @PostMapping
    public ResponseEntity<ApiResponse<CmBltnFile>> create(@RequestBody CmBltnFile entity) {
        CmBltnFile result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<CmBltnFile>> save(
            @PathVariable String id, @RequestBody CmBltnFile entity) {
        entity.setBlogImgId(id);
        CmBltnFile result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
