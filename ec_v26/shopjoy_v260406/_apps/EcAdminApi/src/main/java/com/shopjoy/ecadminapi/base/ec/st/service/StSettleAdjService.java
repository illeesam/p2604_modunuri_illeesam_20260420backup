...
public class StSettleAdjService {
    ...
    @Transactional(readOnly = true)
    public PageResult<StSettleAdjDto> getPageData(Map<String, Object> p, int pageNo, int pageSize) {
        p = new HashMap<>(p);
        int offset = (pageNo - 1) * pageSize;
        p.put("limit", pageSize);
        p.put("offset", offset);
        long totalCount = mapper.selectPageCount(p);
        List<StSettleAdjDto> pageList = mapper.selectPageList(p);
        PageResult<StSettleAdjDto> result = PageResult.of(pageList, totalCount, pageNo, pageSize, p);
        return result;
    }
    ...
    public void delete(String id) {
        if (!repository.existsById(id))
            throw new CmBizException("존재하지 않는 StSettleAdj입니다: " + id);
        repository.deleteById(id);
    }

    /** ID 생성: prefix=SEA (st_settle_adj) */
