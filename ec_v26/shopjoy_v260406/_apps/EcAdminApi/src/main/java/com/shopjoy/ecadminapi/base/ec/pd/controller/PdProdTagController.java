...
public class PdProdTagController {
    ...
    public ResponseEntity<ApiResponse<PageResult<PdProdTagDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<PdProdTagDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PdProdTagDto>> getById(@PathVariable String id) {
    ...
    public ResponseEntity<ApiResponse<PdProdTag>> save(
            @PathVariable String id, @RequestBody PdProdTag entity) {
