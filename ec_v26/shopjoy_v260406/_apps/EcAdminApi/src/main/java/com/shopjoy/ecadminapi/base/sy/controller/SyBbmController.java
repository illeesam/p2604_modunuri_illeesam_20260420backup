...
public class SyBbmController {
    ...
    public ResponseEntity<ApiResponse<PageResult<SyBbmDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<SyBbmDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
    ...
    public ResponseEntity<ApiResponse<SyBbm>> create(@RequestBody SyBbm entity) {
        SyBbm result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<SyBbm>> save(
            @PathVariable String id, @RequestBody SyBbm entity) {
        entity.setBbmId(id);
        SyBbm result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
