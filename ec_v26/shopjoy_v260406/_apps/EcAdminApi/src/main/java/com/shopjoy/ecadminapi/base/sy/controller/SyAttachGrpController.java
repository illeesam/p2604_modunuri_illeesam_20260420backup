...
public class SyAttachGrpController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<SyAttachGrpDto>>> list(
        ...
        List<SyAttachGrpDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<SyAttachGrpDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<SyAttachGrpDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<SyAttachGrpDto>> getById(@PathVariable String id) {
        SyAttachGrpDto result = service.getById(id);
    ...
    public ResponseEntity<ApiResponse<SyAttachGrp>> create(@RequestBody SyAttachGrp entity) {
        SyAttachGrp result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<SyAttachGrp>> save(
            @PathVariable String id, @RequestBody SyAttachGrp entity) {
        entity.setAttachGrpId(id);
        SyAttachGrp result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
