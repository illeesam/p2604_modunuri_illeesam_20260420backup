...
public class StSettleRawController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<StSettleRawDto>>> list(
        ...
        List<StSettleRawDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<StSettleRawDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<StSettleRawDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
    ...
    public ResponseEntity<ApiResponse<StSettleRaw>> create(@RequestBody StSettleRaw entity) {
        StSettleRaw result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<StSettleRaw>> save(
            @PathVariable String id, @RequestBody StSettleRaw entity) {
        entity.setSettleRawId(id);
        StSettleRaw result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
