...
public class PmCouponUsageController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<PmCouponUsageDto>>> list(
        ...
        List<PmCouponUsageDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<PmCouponUsageDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<PmCouponUsageDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PmCouponUsageDto>> getById(@PathVariable String id) {
        PmCouponUsageDto result = service.getById(id);
    ...
    public ResponseEntity<ApiResponse<PmCouponUsage>> create(@RequestBody PmCouponUsage entity) {
        PmCouponUsage result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<PmCouponUsage>> save(
            @PathVariable String id, @RequestBody PmCouponUsage entity) {
        entity.setUsageId(id);
        PmCouponUsage result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
