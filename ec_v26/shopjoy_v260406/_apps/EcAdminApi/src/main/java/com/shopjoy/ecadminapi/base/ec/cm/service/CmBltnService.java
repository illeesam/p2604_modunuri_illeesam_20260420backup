...
public class CmBltnService {
    ...
    public CmBltnDto getById(String id) {
        CmBltnDto result = mapper.selectById(id);
        return result;
    }

    @Transactional(readOnly = true)
    public List<CmBltnDto> getList(Map<String, Object> p) {
        List<CmBltnDto> result = mapper.selectList(p);
        return result;
    }
    ...
    public PageResult<CmBltnDto> getPageData(Map<String, Object> p, int pageNo, int pageSize) {
        ...
        List<CmBltnDto> pageList = mapper.selectPageList(p);
        PageResult<CmBltnDto> result = PageResult.of(pageList, totalCount, pageNo, pageSize, p);
        return result;
    }

    @Transactional
    public int update(CmBltn entity) {
    ...
    public CmBltn create(CmBltn entity) {
        ...
        CmBltn result = repository.save(entity);
        return result;
    }

    @Transactional
    public CmBltn save(CmBltn entity) {
        if (!repository.existsById(entity.getBlogId()))
            throw new CmBizException("존재하지 않는 CmBltn입니다: " + entity.getBlogId());
    ...
    public void delete(String id) {
        if (!repository.existsById(id))
            throw new CmBizException("존재하지 않는 CmBltn입니다: " + id);
        repository.deleteById(id);
    }

    /** ID 생성: prefix=BL (cm_bltn) */
