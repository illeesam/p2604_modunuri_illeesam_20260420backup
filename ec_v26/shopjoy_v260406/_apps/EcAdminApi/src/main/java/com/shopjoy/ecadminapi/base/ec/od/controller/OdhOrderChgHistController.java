...
public class OdhOrderChgHistController {
    ...
    public ResponseEntity<ApiResponse<PageResult<OdhOrderChgHistDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<OdhOrderChgHistDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<OdhOrderChgHistDto>> getById(@PathVariable String id) {
