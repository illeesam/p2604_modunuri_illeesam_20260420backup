...
public class PmDiscntItemService {
    ...
    @Transactional(readOnly = true)
    public PageResult<PmDiscntItemDto> getPageData(Map<String, Object> p, int pageNo, int pageSize) {
        p = new HashMap<>(p);
        int offset = (pageNo - 1) * pageSize;
        p.put("limit", pageSize);
        p.put("offset", offset);
        long totalCount = mapper.selectPageCount(p);
        List<PmDiscntItemDto> pageList = mapper.selectPageList(p);
        PageResult<PmDiscntItemDto> result = PageResult.of(pageList, totalCount, pageNo, pageSize, p);
        return result;
    }
    ...
    /** ID 생성: prefix=DII (pm_discnt_item) */
