...
public class PmDiscntUsageController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<PmDiscntUsageDto>>> list(
        ...
        List<PmDiscntUsageDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<PmDiscntUsageDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<PmDiscntUsageDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PmDiscntUsageDto>> getById(@PathVariable String id) {
        PmDiscntUsageDto result = service.getById(id);
    ...
    public ResponseEntity<ApiResponse<PmDiscntUsage>> create(@RequestBody PmDiscntUsage entity) {
        PmDiscntUsage result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<PmDiscntUsage>> save(
            @PathVariable String id, @RequestBody PmDiscntUsage entity) {
        entity.setDiscntUsageId(id);
        PmDiscntUsage result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
