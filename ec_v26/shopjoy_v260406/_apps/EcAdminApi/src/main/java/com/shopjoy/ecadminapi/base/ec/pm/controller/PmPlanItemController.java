...
public class PmPlanItemController {
    ...
    public ResponseEntity<ApiResponse<PageResult<PmPlanItemDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<PmPlanItemDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PmPlanItemDto>> getById(@PathVariable String id) {
    ...
    public ResponseEntity<ApiResponse<PmPlanItem>> save(
            @PathVariable String id, @RequestBody PmPlanItem entity) {
