...
public class OdDlivItemController {
    ...
    public ResponseEntity<ApiResponse<PageResult<OdDlivItemDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<OdDlivItemDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<OdDlivItemDto>> getById(@PathVariable String id) {
    ...
    public ResponseEntity<ApiResponse<OdDlivItem>> save(
            @PathVariable String id, @RequestBody OdDlivItem entity) {
