...
public class OdClaimItemController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<OdClaimItemDto>>> list(
        ...
        List<OdClaimItemDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<OdClaimItemDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<OdClaimItemDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<OdClaimItemDto>> getById(@PathVariable String id) {
        OdClaimItemDto result = service.getById(id);
    ...
    public ResponseEntity<ApiResponse<OdClaimItem>> create(@RequestBody OdClaimItem entity) {
        OdClaimItem result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<OdClaimItem>> save(
            @PathVariable String id, @RequestBody OdClaimItem entity) {
        entity.setClaimItemId(id);
        OdClaimItem result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
