...
public class OdPayService {
    ...
    @Transactional(readOnly = true)
    public PageResult<OdPayDto> getPageData(Map<String, Object> p, int pageNo, int pageSize) {
        p = new HashMap<>(p);
        int offset = (pageNo - 1) * pageSize;
        p.put("limit", pageSize);
        p.put("offset", offset);
        long totalCount = mapper.selectPageCount(p);
        List<OdPayDto> pageList = mapper.selectPageList(p);
        PageResult<OdPayDto> result = PageResult.of(pageList, totalCount, pageNo, pageSize, p);
        return result;
    }
