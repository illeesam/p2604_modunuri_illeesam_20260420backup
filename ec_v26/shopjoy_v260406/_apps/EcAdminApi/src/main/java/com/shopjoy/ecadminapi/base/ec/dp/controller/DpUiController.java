...
public class DpUiController {
    ...
    public ResponseEntity<ApiResponse<PageResult<DpUiDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<DpUiDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
    ...
    public ResponseEntity<ApiResponse<DpUi>> create(@RequestBody DpUi entity) {
        DpUi result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<DpUi>> save(
            @PathVariable String id, @RequestBody DpUi entity) {
        entity.setUiId(id);
        DpUi result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
