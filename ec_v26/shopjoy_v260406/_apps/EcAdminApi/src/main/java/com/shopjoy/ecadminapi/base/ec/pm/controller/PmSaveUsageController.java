...
public class PmSaveUsageController {
    ...
    public ResponseEntity<ApiResponse<PageResult<PmSaveUsageDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<PmSaveUsageDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PmSaveUsageDto>> getById(@PathVariable String id) {
    ...
    public ResponseEntity<ApiResponse<PmSaveUsage>> save(
            @PathVariable String id, @RequestBody PmSaveUsage entity) {
