...
public class PmSaveUsageService {
    ...
    @Transactional(readOnly = true)
    public PageResult<PmSaveUsageDto> getPageData(Map<String, Object> p, int pageNo, int pageSize) {
        p = new HashMap<>(p);
        int offset = (pageNo - 1) * pageSize;
        p.put("limit", pageSize);
        p.put("offset", offset);
        long totalCount = mapper.selectPageCount(p);
        List<PmSaveUsageDto> pageList = mapper.selectPageList(p);
        PageResult<PmSaveUsageDto> result = PageResult.of(pageList, totalCount, pageNo, pageSize, p);
        return result;
    }
    ...
    public PmSaveUsage create(PmSaveUsage entity) {
        entity.setSaveUsageId(generateId());
    ...
    /** ID 생성: prefix=SAU (pm_save_usage) */
