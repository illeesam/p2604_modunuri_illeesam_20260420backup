...
public class OdOrderDiscntController {
    ...
    public ResponseEntity<ApiResponse<PageResult<OdOrderDiscntDto>>> page(
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<OdOrderDiscntDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<OdOrderDiscntDto>> getById(@PathVariable String id) {
    ...
    public ResponseEntity<ApiResponse<OdOrderDiscnt>> save(
            @PathVariable String id, @RequestBody OdOrderDiscnt entity) {
