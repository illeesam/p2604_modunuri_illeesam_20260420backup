...
public class OdPayController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<OdPayDto>>> list(
        ...
        List<OdPayDto> result = service.getList(p);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/page")
    public ResponseEntity<ApiResponse<PageResult<OdPayDto>>> page(
            @RequestParam(required = false) String siteId,
            @RequestParam(required = false) String kw,
            @RequestParam(required = false) String dateStart,
            @RequestParam(required = false) String dateEnd,
        ...
        Map<String, Object> p = buildParam(siteId, kw, dateStart, dateEnd, sort);
        PageResult<OdPayDto> result = service.getPageData(p, pageNo, pageSize);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<OdPayDto>> getById(@PathVariable String id) {
        OdPayDto result = service.getById(id);
    ...
    public ResponseEntity<ApiResponse<OdPay>> create(@RequestBody OdPay entity) {
        OdPay result = service.create(entity);
        return ResponseEntity.status(201).body(ApiResponse.created(result));
    }

    /* ── 전체 수정 (JPA) ── */
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<OdPay>> save(
            @PathVariable String id, @RequestBody OdPay entity) {
        entity.setPayId(id);
        OdPay result = service.save(entity);
        return ResponseEntity.ok(ApiResponse.ok(result));
    }
