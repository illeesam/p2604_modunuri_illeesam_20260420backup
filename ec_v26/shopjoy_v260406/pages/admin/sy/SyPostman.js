...
window.SyPostman = {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      try {
        const res = await window.adminApi.get('xs/sample07.json');
        (res.data || []).forEach(n => treeRoot.push(makeNode(n)));
        treeLoaded.value = true;
        treeRoot.push(buildAutoCrudNodes());
        treeRoot.push(buildAutoCrudRestNodes());
      } catch {
        treeRoot.push(makeNode({ id:'err', label:'데이터 로딩 실패', type:'folder', open:false, appId:'admin' }));
        treeLoaded.value = true;
      }
    });
    ...
    <div v-show="settingsOpen" style="border-bottom:2px solid #e8587a;background:#fff;flex-shrink:0;padding:10px 14px 12px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
        ...
        <div>
          ...
          <div v-if="token" style="font-size:10px;color:#e8587a;margin-top:3px;">✔ Authorization 헤더에 자동 추가</div>
        </div>
      </div>
            ...
            <div style="flex-shrink:0;padding:12px 14px 0;border-bottom:1px solid #f0f0f0;">
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;">
                ...
                <template v-else-if="histResStatus">
                  ...
                  <span style="font-size:9px;color:#e8587a;background:#ffe4ec;padding:1px 5px;border-radius:3px;font-weight:600;">재전송</span>
                </template>
