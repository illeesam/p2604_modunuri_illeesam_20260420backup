...
window.XsSample09 = {
  ...
  setup() {
    ...
    onMounted(async () => {
      try { const r = await window.frontApi.get('xs/sample09.json'); (r.data || []).forEach(d => allData.push(d)); loadGrid(); }
...
<div style="padding:clamp(12px,3vw,24px);">
  ...
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:12px 16px;margin-bottom:8px;">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
      <input v-model="searchKw" placeholder="질문 / 작성자 검색" @keyup.enter="onSearch" style="font-size:12px;padding:5px 10px;border:1px solid #ddd;border-radius:6px;width:200px;outline:none;" />
      <select v-model="searchCategory" style="font-size:12px;padding:5px 8px;border:1px solid #ddd;border-radius:6px;">
        <option value="">카테고리 전체</option><option>배송</option><option>결제</option><option>반품</option><option>회원</option><option>상품</option>
      </select>
  ...
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-bottom:1px solid #f0f0f0;">
      <span style="font-size:12px;font-weight:700;">FAQ 목록 <span style="color:#e8587a;margin-left:4px;">{{ total }}건</span></span>
    ...
    <div style="overflow-x:auto;">
      <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:780px;">
        ...
        <tbody>
          ...
          <tr v-for="(row,idx) in pagedRows" :key="row.faqId"
            ...
            <td style="text-align:center;">
              <select v-model="row.category" :disabled="row._row_status==='D'" @change="onCellChange(row)" style="font-size:11px;padding:2px 4px;border:1px solid #ddd;border-radius:4px;background:#fff;">
                <option>배송</option><option>결제</option><option>반품</option><option>회원</option><option>상품</option>
              </select>
            </td>
