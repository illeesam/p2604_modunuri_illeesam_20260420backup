...
window.XsSample05 = {
  ...
  setup() {
    ...
    const makeRow = d => ({ ...d, _row_status: 'N', _row_check: false, _orig: { title: d.title, author: d.author, category: d.category, status: d.status } });
    ...
    onMounted(async () => {
      try { const r = await window.frontApi.get('xs/sample05.json'); (r.data || []).forEach(d => allData.push(d)); loadGrid(); }
    ...
    const addRow = () => {
      ...
      gridRows.splice(at, 0, { boardId: _tempId--, title: '', author: '', category: '공지', viewCnt: 0, status: '공개', regDate: '', _row_status: 'I', _row_check: false, _orig: null });
    ...
    const cancelChecked = () => { const ids = new Set(gridRows.filter(r => r._row_check).map(r => r.boardId)); if (!ids.size) { showToast('취소할 행을 선택해주세요.', 'info'); return; } for (let i = gridRows.length - 1; i >= 0; i--) { const row = gridRows[i]; if (!ids.has(row.boardId)) continue; if (row._row_status === 'N') continue; if (row._row_status === 'I') gridRows.splice(i, 1); else { if (row._orig) EDIT_FIELDS.forEach(f => { row[f] = row._orig[f]; }); row._row_status = 'N'; } } };
    ...
    const doSave = async () => {
      ...
      dRows.forEach(r => { const i = allData.findIndex(d => d.boardId === r.boardId); if (i !== -1) allData.splice(i, 1); });
      uRows.forEach(r => { const i = allData.findIndex(d => d.boardId === r.boardId); if (i !== -1) EDIT_FIELDS.forEach(f => { allData[i][f] = r[f]; }); });
      let nid = Math.max(...allData.map(d => d.boardId), 0);
      iRows.forEach(r => { allData.push({ boardId: ++nid, title: r.title, author: r.author, category: r.category, viewCnt: 0, status: r.status, regDate: new Date().toISOString().slice(0, 10) }); });
...
<div style="padding:clamp(12px,3vw,24px);">
  ...
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-bottom:1px solid #f0f0f0;">
      <span style="font-size:12px;font-weight:700;">게시판 목록 <span style="color:#e8587a;margin-left:4px;">{{ total }}건</span></span>
    ...
    <div style="overflow-x:auto;">
      <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:760px;">
        ...
        <tbody>
          ...
          <tr v-for="(row,idx) in pagedRows" :key="row.boardId"
